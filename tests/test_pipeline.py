...
class TestDataSchema:
    ...
    def test_poids_positive(self, clean_df):
        # Après StandardScaler, valeurs centrées-réduites (mean≈0, std≈1)
        # Les valeurs négatives sont normales — on vérifie la normalisation
        mean = clean_df["Poids"].mean()
        std = clean_df["Poids"].std()
        assert abs(mean) < 0.1, f"Mean Poids attendu ≈ 0, obtenu : {mean:.4f}"
        assert abs(std - 1) < 0.1, f"Std Poids attendu ≈ 1, obtenu : {std:.4f}"
    ...
    def test_volume_positive(self, clean_df):
        ...
        assert abs(mean) < 0.1, f"Mean Volume attendu ≈ 0, obtenu : {mean:.4f}"
        assert abs(std - 1) < 0.1, f"Std Volume attendu ≈ 1, obtenu : {std:.4f}"
...
class TestNLPPipeline:
    ...
    def test_pretraiter_texte_removes_stopwords(self):
        ...
        from src.nlp.train_nlp import get_stopwords, pretraiter_texte
    ...
    def test_pretraiter_non_empty_text(self):
        ...
        stops = get_stopwords()
        stemmer = SnowballStemmer("french")
        result = pretraiter_texte(
            "fragment brillant conducteur transparent flexible", stops, stemmer
        )
...
class TestModelPredictions:
    ...
    def test_predict_output_is_known_class(self, loaded_classifier, loaded_scaler):
        ...
        le = (
            joblib.load("models/le_categorie.pkl")
            if os.path.exists("models/le_categorie.pkl")
            else None
        )
        sample = np.array([[1.5, 2.0, 0.8, 0.6, 0.7]])
        scaled = loaded_scaler.transform(sample)

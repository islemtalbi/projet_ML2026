...
Détection de data drift numérique + text drift (Jensen-Shannon)
"""
...
def run_numeric_drift_report(ref_path: str, cur_path: str, output_dir: str = REPORTS_DIR):
    ...
    try:
        ...
        print(f"[evidently] Rapport sauvegardé → {html_path}")
        print(f"[evidently] JSON sauvegardé    → {json_path}")
        ...
        print("[evidently] Evidently non installé — utilisation du mode manuel")
    ...
    try:
        ...
        for m in metrics:
            if m.get("metric") == "DatasetDriftMetric":
                ...
                print(f"\n[drift] Dataset drift détecté : {'OUI ⚠️' if drift else 'NON ✅'}")
                print(f"[drift] Colonnes driftées : {n_drifted}/{n_total}")
    except Exception:
        pass


# ── Drift numérique manuel (sans Evidently) ───────────────────────────────────
...
def run_manual_drift(ref_path: str, cur_path: str, output_dir: str = REPORTS_DIR) -> dict:
    ...
    print("\n=== RAPPORT DRIFT NUMÉRIQUE (Manuel) ===")
    ...
    print(f"\n[résumé] {n_drifted}/{len(NUMERIC_FEATURES)} colonnes avec drift")
...
def run_text_drift(ref_path: str, cur_path: str, output_dir: str = REPORTS_DIR) -> dict:
    ...
    # TF-IDF sur les deux corpus combinés
    ...
    print(f"[text drift] {'⚠️ DRIFT DÉTECTÉ' if drifted else '✅ Pas de drift'} (seuil={THRESHOLD})")
...
def check_alerts(numeric_report: dict, text_report: dict = None):
    ...
    if n_drifted > 0:
        alerts.append(f"⚠️  {n_drifted} colonne(s) numérique(s) en drift")

    if text_report and text_report.get("text_drift"):
        alerts.append(f"⚠️  Text drift détecté (JS={text_report['js_divergence']:.4f})")
    ...
    if alerts:
        ...
        print("\n✅ Aucune alerte — données en production stables")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reference", default="data/processed/dataset_clean.csv",
                        help="Dataset de référence (train)")
